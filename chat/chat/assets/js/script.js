...
// ===== CONFIGURACIÓN DE CHATPDF =====
const pdfSources = {
  semana1: "vkccaBc8SpkPFIIOa3zIq",
  "mobile-semana1": "CAS9pUvaZK8CGXcXVqzKb",
}

// ===== CONFIGURACIÓN DE IA GENERAL (NO USADA CON CHATPDF) =====
const API_KEY = "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"
...
const usersDB = [
  ...
  { email: "admin@autonoma.edu.pe", password: "admin123", name: "Administrador", role: "admin" },
]
...
function handleLogin() {
  ...
  // Siempre usar el rol de estudiante para nuevos usuarios, pero mantener el rol real del usuario en la base de datos
  ...
  userEmail = email
  userName = user.name
  userRole = user.role // Usar el rol real del usuario desde la base de datos
...
function selectTopic(topicId) {
  ...
  if (selectedTopic) {
    ...
    // Mostrar/ocultar la sección de subida de PDF según el rol del usuario
...
async function sendChatMessage() {
  ...
  if (!message || !selectedTopic || !pdfSources[selectedTopic.id]) {
    chatError.textContent = "La informacion que estas consultando no esta realcionado al silabus del curso."
    return
  }
 
  const sourceId = pdfSources[selectedTopic.id]
  ...
  try {
    const response = await fetch("https://api.chatpdf.com/v1/chats/message", {
      ...
      headers: {
        "Content-Type": "application/json",
        "x-api-key": "sec_mMaCjawLIKly4WjldelATiKlPbOvPD2l",
      },
      body: JSON.stringify({
        sourceId: sourceId,
    ...
    aiMessageDiv.textContent = reply
    chatMessages.appendChild(aiMessageDiv)

    chatMessages.scrollTop = chatMessages.scrollHeight
  } catch (error) {
    console.error("Error con ChatPDF:", error)
    chatError.textContent = "Error al enviar tu mensaje. Intenta de nuevo."
  } finally {
...
// ===== SUBIDA DE PDF SOLO VISIBLE PARA ADMINISTRADOR =====
async function uploadPDFToChatPDF(file) {
  ...
  if (!response.ok) {
    throw new Error("Error al subir el PDF a ChatPDF")
  }
...
async function handlePDFUpload() {
  ...
  if (!selectedTopic || !selectedTopic.id) {
    alert("Selecciona un tema antes de subir el PDF.")
    return
  }

  try {
    const sourceId = await uploadPDFToChatPDF(file)
    pdfSources[selectedTopic.id] = sourceId

    const resultBox = document.getElementById("source-id-result")
    resultBox.textContent = `✅ PDF subido y asociado al tema "${selectedTopic.id}". sourceId: ${sourceId}`
